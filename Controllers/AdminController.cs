...
public class AdminController : Controller
{
    ...
    public IActionResult RecoverPassword(User postedData)
    {
        ...
        if (user != null)
        {
            ...
            TempData["Success"] = "Şifreniz e-mail adresinize gönderilmiştir.";
            return Redirect("/admin/forgot-password");
        }
        else
        {
            TempData["Danger"] = "Bu e-mail adresi sisteme kayıtlı değildir!";
            return Redirect("/admin/forgot-password");
        }
    }
    ...
    {
        int uid = int.Parse(User.Claims.FirstOrDefault(x => x.Type == "id")!.Value);

        var model = new IndexViewModel()
        {
            User = db.Users!.FirstOrDefault(x => x.Id == uid && x.Role == "admin"),
            Site = db.Sites!.First(),
        };
    ...
    public IActionResult SlideEdit(int id)
    {
        ...
        {
            User = db.Users!.FirstOrDefault(x => x.Id == uid && x.Role == "admin"),
            Slide = db.Slides!.FirstOrDefault(x => x.Id == id),
            Site = db.Sites!.First(),
        };
    ...
    public IActionResult SlideDetail(int id)
    {
        int uid = int.Parse(User.Claims.FirstOrDefault(x => x.Type == "id")!.Value);
    ...
    public IActionResult SlideSave(Slide postedData, IFormFile File)
    {
        if (postedData.Id != 0)
        {
            Slide toUpdate = db.Slides.FirstOrDefault(x => x.Id == postedData.Id)!;
            if (File != null)
            {
                string fileExtension = System.IO.Path.GetExtension(File.FileName);
                string uploadsFolder = Path.Combine(webHostEnvironment.WebRootPath, "uploads/images/");
                string fileName = Methods.GenerateUrl(postedData.Title!) + "-slide" + fileExtension;
